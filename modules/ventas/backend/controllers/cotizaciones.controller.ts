...
export class CotizacionesController {
  ...
  @ApiOperation({
    summary: 'Listar catálogo de estados de cotización',
    description: `Requiere ${PERMISO_GESTIONAR}. Catálogo de solo lectura (\`quote_status\`) — declarado antes de ":id" para no colisionar con esa ruta.`,
  })
  @RequirePermission(PERMISO_GESTIONAR)
  @Get('estados')
  async listarEstados(@CurrentUser() user: UserContext) {
    const result = await this.cotizacionesService.listarEstados(user);
    return { data: result.data, meta: result.meta };
  }
