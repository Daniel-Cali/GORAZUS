...
describe('Notas de Crédito de Compra (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const empresa = await prisma.companies.create({
      data: {
        id: randomUUID(),
        tenant_id: tenant.id,
        legal_name: `Empresa e2e NC ${Date.now()}`,
        tax_id: `NIT-NC-${Date.now()}`,
    ...
    const payload: Omit<AccessTokenPayload, 'iat' | 'exp'> = {
      ...
      companyId,
      branchId: null,
      sessionId: 'e2e-test-session-compras-nc',
    };
    ...
    const proveedor = await suppliersPrisma.suppliers.create({
      data: {
        ...
        tenant_id: tenant.id,
        company_id: companyId,
        legal_name: `Proveedor e2e NC ${Date.now()}`,
        tax_id: `RNC-NC-${Date.now()}`,
      },
    });
  ...
  it('flujo completo: crear factura → nota de crédito parcial (monto calculado) → exceder rechazado → completar exacto → anular', async () => {
    const crearFactura = await request(app.getHttpServer())
      ...
      .send({
        companyId,
        supplierId,
        supplierDocumentNumber: `FACT-NC-${Date.now()}`,
        currencyCode: 'USD',
        lines: [{ productId, quantity: 10, unitCost: 15 }],
      });
    const facturaId = crearFactura.body.data.id;

    const nota1 = await request(app.getHttpServer())
      .post('/api/v1/compras/notas-credito')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ purchaseInvoiceId: facturaId, lines: [{ productId, quantity: 6 }] });
    expect(nota1.status).toBe(201);
    ...
    // '90', no '90.0000'.
    expect(nota1.body.data.total_amount).toBe('90'); // 6 × 15
    ...
    const nota2 = await request(app.getHttpServer())
      .post('/api/v1/compras/notas-credito')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ purchaseInvoiceId: facturaId, lines: [{ productId, quantity: 4 }] }); // 6+4 = 10
    expect(nota2.status).toBe(201);

    const anular = await request(app.getHttpServer())
      .post(`/api/v1/compras/notas-credito/${nota2.body.data.id}/anular`)
      .set('Authorization', `Bearer ${adminToken}`);
    expect(anular.status).toBe(201);
  });

  it('POST /compras/notas-credito contra una factura cancelada devuelve 409', async () => {
    const crearFactura = await request(app.getHttpServer())
      .post('/api/v1/compras/facturas')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({
        companyId,
        supplierId,
        supplierDocumentNumber: `FACT-NC-CANC-${Date.now()}`,
        currencyCode: 'USD',
        lines: [{ productId, quantity: 5, unitCost: 15 }],
      });
    const facturaId = crearFactura.body.data.id;
