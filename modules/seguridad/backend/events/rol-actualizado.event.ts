/** Domain Event — Roles Enterprise, Subfase 4.1 (preparado, sin publicar todavía). Ver `rol-creado.event.ts` para el porqué de "preparado, no publicado". */
export class RolActualizadoEvent {
  static readonly routingKey = 'seguridad.rol.actualizado';

  constructor(
    public readonly rolId: string,
    public readonly tenantId: string,
    public readonly nombreAnterior: string,
    public readonly nombreNuevo: string,
    public readonly ocurridoEn: Date,
  ) {}

  toPayload(): Record<string, unknown> {
    return {
      rolId: this.rolId,
      tenantId: this.tenantId,
      nombreAnterior: this.nombreAnterior,
      nombreNuevo: this.nombreNuevo,
      ocurridoEn: this.ocurridoEn.toISOString(),
    };
  }
}
