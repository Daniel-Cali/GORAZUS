...
> Fase 05 Parte 02 — motor de stock y movimientos real de
...
`productos` — solo el producto base), con el núcleo de identidad/
...
con multiempresa) — Fase 05 Parte 02 (Motor de Stock y Movimientos) del
orden de desarrollo queda cerrada con esta parte.

## 2. Versión actual

**0.8.0** (2026-07-23) — FASE 05, Parte 02: Motor de Stock y
Movimientos — catálogo de tipos de movimiento, registro de movimientos
con actualización atómica de `inventory.stock`, disponible y kardex
real (`inventory.v_kardex`). Ver [VERSION.md](./VERSION.md) para el
historial completo de versiones y
[INVENTORY_STOCK_REPORT.md](./INVENTORY_STOCK_REPORT.md) para el
...
| Infraestructura Docker (Postgres, Redis, RabbitMQ, MinIO, nginx, pgAdmin, MailHog, Prometheus/Grafana/Loki)  | ✅ verificada de punta a punta en su momento — Docker caído 7 sesiones seguidas |
...
| Inventario — Motor de stock y movimientos                                                                    | ✅ FASE 05 Parte 02 — motor real (stock/tipos de movimiento/movimientos/kardex) |
...
| Testing (unitario/integración/e2e/carga/seguridad)                                                           | ✅ 298+ tests reales — ver nota de disponibilidad de Docker en `TEST_REPORT.md` |
...
**primer consumidor de aplicación** para 3 tablas más del schema
`inventory` (`stock`/`stock_movement_types`/`stock_movements`, 6 de 34
en total con Almacenes) y el **primer uso de una vista de Postgres**
desde código de aplicación en todo el proyecto (`inventory.v_kardex`,
vía `$queryRawUnsafe` parametrizado) — sin tocar el schema en sí.

## 6. Orden de desarrollo — Fase 05 Parte 02 cerrada

Con el motor de stock y movimientos completo, la Parte 02 de la Fase 05
del orden de desarrollo queda cerrada. La lista de prioridad "primero"
de FASE 03 y la Fase 04 (Productos) ya estaban 100% cubiertas — ver
...
`inventario` mismo, 28 de las 34 tablas del schema siguen sin código
(reservas/transferencias/ajustes/conteos/recepciones/salidas/costeo/
series/lotes/producción), ya diseñadas en `INVENTORY_ARCHITECTURE.md`.
Orden de construcción confirmado (`INVENTORY_NEXT_PHASE.md`): **Fase 05
Parte 03 — Reservas y Transferencias** → 04 Ajustes/conteos → 05
Recepciones/salidas/reglas de almacén → 06 Costeo → 07 Series/lotes →
...
duplicado acá. Se suma un ítem nuevo de esta parte: 6 gaps de schema
detectados en el diseño de Inventario (QR/RFID, fecha de fabricación,
peso/volumen/dimensiones, obsolescencia, garantías, trazabilidad de
caja) — ver `INVENTORY_ARCHITECTURE.md §5.2`, requieren decisión de
negocio antes de una migración futura, no bloquean el trabajo actual.
