...
> Fase 05 Parte 03 — Reservas y Transferencias reales de
...
Reservas/Transferencias, `productos` — solo el producto base), con el
núcleo de identidad/administración ya endurecido para producción (2FA
exigido, bloqueo por intentos, revocación de sesión, session-hijacking,
verificación de empresa/sucursal activa, CSRF, email real, gestión de
usuarios completa con multiempresa) — Fase 05 Parte 03 (Reservas y
Transferencias) del orden de desarrollo queda cerrada con esta parte.

## 2. Versión actual

**0.9.0** (2026-07-23) — FASE 05, Parte 03: Reservas y Transferencias
— reservas que protegen stock sin descontarlo, transferencias entre
almacenes con flujo de estados completo (`draft → in_transit →
received`) y movimientos atómicos multi-línea
(`MovimientoStockRepository.registrarLote`). Corrige el chequeo de
stock suficiente de `0.8.0` para comparar contra disponible real. Ver
[VERSION.md](./VERSION.md) para el historial completo de versiones y
[INVENTORY_RESERVAS_TRANSFERENCIAS_REPORT.md](./INVENTORY_RESERVAS_TRANSFERENCIAS_REPORT.md)
para el detalle de esta parte.
...
| Inventario — Motor de stock, movimientos, reservas y transferencias                                          | ✅ FASE 05 Parte 02+03 — motor real completo                                    |
...
| Testing (unitario/integración/e2e/carga/seguridad)                                                           | ✅ 332+ tests reales — ver nota de disponibilidad de Docker en `TEST_REPORT.md` |
...
`inventory` (`stock_reservations`/`stock_transfers`/
`stock_transfer_lines`, 9 de 34 en total) — sin tocar el schema en sí.

## 6. Orden de desarrollo — Fase 05 Parte 03 cerrada

Con Reservas y Transferencias completas, la Parte 03 de la Fase 05 del
...
`inventario` mismo, 25 de las 34 tablas del schema siguen sin código
(ajustes/conteos/recepciones/salidas/costeo/series/lotes/producción),
ya diseñadas en `INVENTORY_ARCHITECTURE.md`. Orden de construcción
confirmado (`INVENTORY_NEXT_PHASE.md`): **Fase 05 Parte 04 — Ajustes y
Conteos Físicos** → 05 Recepciones/salidas/reglas de almacén → 06
Costeo → 07 Series/lotes → 08 Producción → Clientes → Ventas → Caja →
POS.
...
`INVENTORY_ARCHITECTURE.md §5.2`. Nuevo de esta parte: cancelar una
transferencia ya `in_transit` no está soportado — requeriría un
movimiento de reversión no especificado en el pedido original, decisión
de producto pendiente si se necesita.
