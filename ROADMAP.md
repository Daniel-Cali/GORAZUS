...
| `ventas`                                                                                                                                                                                    | 🟡 Motor de Facturación Enterprise Parte 1 + Módulo de Ventas Enterprise Parte 1: Cotizaciones (crear/editar/aprobar/rechazar/duplicar/vigencia), Pedidos (reserva real de inventario, conversión total/parcial a factura), Facturación completa — 13 de 55 tablas. Comprobantes fiscales/Listas de precio/Descuentos avanzados/Devoluciones/Entregas/Comisiones sin código, ver `docs/reports/ventas/SALES_ROADMAP.md`             | ❌ Sin construir                                                                   |
...
## Fase actual: Módulo de Ventas Enterprise, Parte 1 — Cotización → Pedido → Factura (2026-07-27)

`v0.24.0` completa el extremo que le faltaba al motor de facturación (`v0.22.0`): Cotizaciones
(crear/editar/aprobar/rechazar/duplicar/vigencia/historial) y Pedidos de venta (reserva real de
inventario vía `ReservasService`, recién expuesto de `InventarioModule`; conversión total o
parcial a factura, reutilizando el motor de Facturación existente). Cadena verificada de punta a
punta contra Postgres real: Cotización → Pedido → Factura → Asiento contable (conecta las tres
partes construidas en esta sesión: Facturación, Contabilidad y Ventas Pipeline).

Origen: pedido "PROMPT MAESTRO — MÓDULO DE VENTAS ENTERPRISE" con 15 secciones (alcance de varias
semanas) — se acordó con el usuario el orden de partes antes de escribir código. Comprobantes
fiscales/NCF, listas de precio, descuentos/promociones avanzados, devoluciones (gap de severidad
alta ya documentado), entregas y comisiones quedan fuera de esta parte. Migración aditiva
(`41_ventas_pedidos_facturacion_parcial.sql`): `sales_order_lines.invoiced_quantity`. Ver
`docs/reports/ventas/SALES_PIPELINE_ARCHITECTURE.md` para el detalle completo,
`SALES_API_REPORT.md` para endpoints, `SALES_TEST_REPORT.md` para testing,
`SALES_ROADMAP.md` para las secciones restantes.

## Fase anterior: Contabilidad Enterprise, Parte 1 — Núcleo Contable + Estados Financieros (2026-07-27)
